-- Restore get_portfolio_page after 026 referenced alias vr_sort inside an
-- expression (Postgres error: column "vr_sort" does not exist). This is the
-- last known-good function. Run this in the SQL editor to bring the site back.
...
begin
  ...
  select coalesce(
    jsonb_agg(
      ...
      order by vr_sort desc nulls last, video_sort desc nulls last, sort_order asc, name asc
    ),
    '[]'::jsonb
  )
  into v_items

-- State + builder/project/city labels on portfolio items (state is primary filter)
...
create index if not exists portfolio_items_state_idx
  on public.portfolio_items (state);

-- Backfill from legacy city join
update public.portfolio_items pi
set
  state = coalesce(nullif(trim(pi.state), ''), c.state),
  project_name = coalesce(nullif(trim(pi.project_name), ''), pi.name),
  city_label = coalesce(nullif(trim(pi.city_label), ''), c.name)
from public.cities c
where pi.city_id = c.id;
...
begin
  ...
  from (
    ...
    order by pi.sort_order asc, coalesce(nullif(trim(pi.project_name), ''), pi.name) asc
    limit v_page_size
    offset v_offset
  ) t;

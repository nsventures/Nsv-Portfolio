-- Public Virtual Reality tab follows admin drag order (sort_order).
-- Aliases are only used as plain ORDER BY names — the 026 CASE-on-alias
-- pattern is what caused "column vr_sort does not exist".
-- Videos and the mixed grid are unchanged.
...
begin
  ...
  with page_rows as (
    select
      ...
      case when p_media_type = 'virtual-tour' then pi.sort_order end as tab_sort,
    ...
    order by
      tab_sort asc nulls last,
      vr_sort desc nulls last,
  ...
  select coalesce(
    jsonb_agg(
      ...
      order by tab_sort asc nulls last, vr_sort desc nulls last, video_sort desc nulls last, sort_order asc, name asc
    ),
    '[]'::jsonb
  )
  into v_items

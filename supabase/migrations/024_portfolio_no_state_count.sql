-- Fix totals: expose no-state count; support filtering to unassigned state only.
-- Sentinel: p_state = '__none__' → only items with no state.
...
begin
  ...
  select coalesce(
    jsonb_agg(
      ...
      order by video_sort desc nulls last, sort_order asc, name asc
    ),
    '[]'::jsonb
  )
  into v_items

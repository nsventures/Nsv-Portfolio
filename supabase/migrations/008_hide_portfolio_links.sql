-- Hide VR/video links from public list API; fetch on demand when user opens a viewer.

drop policy if exists "public read published portfolio" on public.portfolio_items;

create or replace function public.get_portfolio_viewer(p_item_id text)
returns jsonb
language plpgsql
stable
security definer
...
declare
  ...
  v_media_type text;
begin
  select pi.link, pi.media_type
  into v_link, v_media_type
  from public.portfolio_items pi
  where pi.id = p_item_id
    and pi.is_published = true;

  if not found or v_link is null or trim(v_link) = '' then
    return null;
  end if;
...
grant execute on function public.get_portfolio_viewer(text) to anon, authenticated;
